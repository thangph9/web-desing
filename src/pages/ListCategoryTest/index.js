          ...
          <div>
            ...
            >
              <img
                ...
                alt="image"
              />
              <div className={styles['product-card__discount___2R4sK']}>
                {Math.round((1 - data.sale.salePrice / data.price) * 100)}% Off
              </div>
            </div>
            <div className={styles['product-card__titleContainer___1HE6o']}>
              <h4 className={styles['product-card__brand___3np4R']}>{data.seller}</h4>
              <h4 className={styles['product-card__title___3l79X']}>{data.name}</h4>
              <div>
...
@Form.create()
class ListCategoryTest extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      filter: false,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.category.searchtest.filterMap !== this.props.category.searchtest.filterMap) {
      var stringSizeTotal = '';
      nextProps.category.searchtest.filterMap.size.forEach(element => {
...
export default ListCategoryTest;
